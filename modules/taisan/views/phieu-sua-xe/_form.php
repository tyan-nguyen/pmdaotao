...
<div class="phieu-de-nghi-form">
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <?= $form->field($model, 'id_tham_chieu')->widget(Select2::class, [
                'data' =>  Xe::getListAll(),
                'language' => 'vi',
                'options' => [
                    'placeholder' => 'Chọn xe...',
        ...
        <?php /* ?>
        <div class="col-md-2">
            <?= $form->field($model, 'tong_tien_du_kien')->textInput(['maxlength' => true]) ?>
        </div>
        <div class="col-md-2">
            <?= $form->field($model, 'tong_tien_thuc_te')->textInput(['maxlength' => true]) ?>
        </div>
        <?php */ ?>
    ...
    <?php if (!$model->isNewRecord) { ?>
                                ...
                                <table id="vtTable" class="table table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:15%">Loại hạng mục</th>
        ...
        <?php /* if ($model->trang_thai == PhieuDeNghi::TRANGTHAI_DADUYET || $model->trang_thai == PhieuDeNghi::TRANGTHAI_HOANTHANH) { ?>
            <a class="btn btn-primary btn-sm" href="/taisan/phieu-sua-xe/thanh-toan?id=<?= $model->id ?>" role="modal-remote"><i class="fa-solid fa-file-export"></i> Thanh toán</a>
...
<script type="text/javascript">
    ...
    function AddVatTu() {
        ...
        formRow += '<td><input type="text" name="ghiChu" style="width:100%" /></td>';
    ...
    function editVatTu(arr) {
        if ($("#idTrUpdate").length > 0) {
            ...
            formRow += '<td><input type="text" name="ghiChu" value="' + arr['ghiChu'] + '" style="width:100%" /></td>';
    ...
    function InHoaDon() {
        ...
        $.ajax({
            type: 'post',
            url: '/taisan/phieu-sua-xe/get-phieu-in-ajax?idPhieu=' + <?= $model->id ? $model->id : "''" ?>,
            //data: frm.serialize(),
            success: function(data) {
                console.log('Submission was successful.');
                console.log(data);
    ...
    function updatePrintCount(id) {
        $.ajax({
            type: 'POST',
            url: '/taisan/phieu-sua-xe/update-print-count?id=' + id,
    ...
    function InHoaDonChiPhi() {
        ...
        $.ajax({
            type: 'post',
            url: '/taisan/phieu-sua-xe/get-phieu-in-chi-phi-ajax?idPhieu=' + <?= $model->id ? $model->id : "''" ?>,
            //data: frm.serialize(),
            success: function(data) {
                console.log('Submission was successful.');
                console.log(data);
