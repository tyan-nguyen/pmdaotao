...
<div class="row" style="margin-top:20px">
    ...
    <div class="col-md-4">
        <label class="form-label">Nơi thu tiền</label>
        <?= Html::dropDownList('byreceivemoney', null, DangKyHv::getDmNoiDangKy(), ['id' => 'byreceivemoney', 'class' => 'form-control', 'prompt' => '-Tất cả-']) ?>
    </div>
</div>
...
<script>
    function InBaoCao(nhap, typereport) {
        $.ajax({
            type: 'POST',
            url: '/hocvien/dang-ky-hv/get-phieu-in-report-list-ajax?startdate=' + $('#startdate').val() + '&starttime=' + $('#starttime').val() + '&enddate=' + $('#enddate').val() + '&endtime=' + $('#endtime').val() + '&byuser=' + $('#byuser').val() + '&typereport=' + typereport + '&byaddress=' + $('#byaddress').val() + '&byreceivemoney=' + $('#byreceivemoney').val(),
    ...
    function XuatExcel(nhap, typereport) {
        var url = '/hocvien/bao-cao/xuat-danh-sach-ca-excel?startdate=' + $('#startdate').val() + '&starttime=' + $('#starttime').val() + '&enddate=' + $('#enddate').val() + '&endtime=' + $('#endtime').val() + '&byuser=' + $('#byuser').val() + '&typereport=' + typereport + '&byaddress=' + $('#byaddress').val() + '&byreceivemoney=' + $('#byreceivemoney').val();
