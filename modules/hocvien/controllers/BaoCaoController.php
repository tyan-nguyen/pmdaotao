...
class BaoCaoController extends Controller
{
     ...
     * xuất excel
     */
    public function actionXuatDanhSachCaExcel($startdate, $starttime, $enddate, $endtime, $byuser, $typereport, $byaddress, $byreceivemoney)
    {
        if ($byuser == null) {
            $byuser = 0;
        }
        ...
        if ($byuser > 0) {
            $thoiGianLabel .= ' - Nhân viên nhận hồ sơ: ' . User::findOne($byuser)->getHoTen();
        } else if ($byreceivemoney) {
            $thoiGianLabel .= ' - Nhân viên nhận hồ sơ: ' . (isset(DangKyHv::getDmNoiDangKyShort()[$byreceivemoney]) ? DangKyHv::getDmNoiDangKyShort()[$byreceivemoney] : '');
        } else {
            $thoiGianLabel .= ' - Nhân viên nhận hồ sơ: Tất cả';
        }
