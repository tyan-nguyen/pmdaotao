...
class HvThueController extends Controller
{
     ...
     * @return mixed
     */
    public function actionSetThue($id)
    {
        $request = Yii::$app->request;
        $model = $this->findModel($id);
        ...
        if($request->isAjax){
            ...
            Yii::$app->response->format = Response::FORMAT_JSON;
            if($model != null){
                $model->co_ho_so_thue = 1;
                $model->updateAttributes(['co_ho_so_thue']);
            }
            return [
                //'forceClose'=>true,
                'forceReload'=>'#crud-datatable-pjax',
                'title'=> "Cập nhật trạng thái <strong>đã xuất</strong> hóa đơn thuế",
                'content'=>'<span class="text-success">Đã cập nhật trạng thái xuất hóa đơn thuế cho học viên !</span>',
                'footer'=> Html::button('Đóng lại',['class'=>'btn btn-default pull-left','data-bs-dismiss'=>"modal"])
            ];
        }else{
            /*
     ...
     * @return mixed
     */
    public function actionCancelSetThue($id)
    {
        $request = Yii::$app->request;
        $model = $this->findModel($id);
        ...
        if($request->isAjax){
            ...
            Yii::$app->response->format = Response::FORMAT_JSON;
            if($model != null){
                $model->co_ho_so_thue = 0;
                $model->updateAttributes(['co_ho_so_thue']);
            }
            return [
                //'forceClose'=>true,
                'forceReload'=>'#crud-datatable-pjax',
                'title'=> "Cập nhật trạng thái xuất hóa đơn thuế",
                'content'=>'<span class="text-danger">Đã cập nhật trạng thái <strong>chưa xuất</strong> hóa đơn thuế cho học viên !</span>',
                'footer'=> Html::button('Đóng lại',['class'=>'btn btn-default pull-left','data-bs-dismiss'=>"modal"])
            ];
        }else{
            /*
