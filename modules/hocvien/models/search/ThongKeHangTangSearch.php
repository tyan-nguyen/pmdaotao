 ...
 */
class ThongKeHangTangSearch extends DangKyHv
{
    ...
    public $ngay_nhan_tai_lieu_tu;
    public $ngay_nhan_tai_lieu_den;
    public $ngay_nhan_ao_tu;
    public $ngay_nhan_ao_den;
    ...
    public function search($params)
    {
        ...
        //ngày nhận tài liệu
        //có cả từ ngày & đến ngày
        if (!empty($this->ngay_nhan_tai_lieu_tu) && !empty($this->ngay_nhan_tai_lieu_den)) {
            $this->ngay_nhan_tai_lieu_tu = CustomFunc::convertDMYToYMD($this->ngay_nhan_tai_lieu_tu);
            $this->ngay_nhan_tai_lieu_den = CustomFunc::convertDMYToYMD($this->ngay_nhan_tai_lieu_den);
            $query->andFilterWhere(['between', 'ngay_nhan_tai_lieu', $this->ngay_nhan_tai_lieu_tu, $this->ngay_nhan_tai_lieu_den]);
        }

        // Nếu chỉ có từ ngày
        if (!empty($this->ngay_nhan_tai_lieu_tu) && empty($this->ngay_nhan_tai_lieu_den)) {
            $this->ngay_nhan_tai_lieu_tu = CustomFunc::convertDMYToYMD($this->ngay_nhan_tai_lieu_tu);
            $query->andFilterWhere(['>=', 'ngay_nhan_tai_lieu', $this->ngay_nhan_tai_lieu_tu]);
        }

        // Nếu chỉ có đến ngày
        if (empty($this->ngay_nhan_tai_lieu_den) && !empty($this->ngay_nhan_tai_lieu_den)) {
            $this->ngay_nhan_tai_lieu_den = CustomFunc::convertDMYToYMD($this->ngay_nhan_tai_lieu_den);
            $query->andFilterWhere(['<=', 'ngay_nhan_tai_lieu', $this->ngay_nhan_tai_lieu_den]);
        }

        //ngày nhận áo
        //có cả từ ngày & đến ngày
        if (!empty($this->ngay_nhan_ao_tu) && !empty($this->ngay_nhan_ao_den)) {
            $this->ngay_nhan_ao_tu = CustomFunc::convertDMYToYMD($this->ngay_nhan_ao_tu);
            $this->ngay_nhan_ao_den = CustomFunc::convertDMYToYMD($this->ngay_nhan_ao_den);
            $query->andFilterWhere(['between', 'ngay_nhan_ao', $this->ngay_nhan_ao_tu, $this->ngay_nhan_ao_den]);
        }

        // Nếu chỉ có từ ngày
        if (!empty($this->ngay_nhan_ao_tu) && empty($this->ngay_nhan_ao_den)) {
            $this->ngay_nhan_ao_tu = CustomFunc::convertDMYToYMD($this->ngay_nhan_ao_tu);
            $query->andFilterWhere(['>=', 'ngay_nhan_ao', $this->ngay_nhan_ao_tu]);
        }

        // Nếu chỉ có đến ngày
        if (empty($this->ngay_nhan_ao_den) && !empty($this->ngay_nhan_ao_den)) {
            $this->ngay_nhan_ao_den = CustomFunc::convertDMYToYMD($this->ngay_nhan_ao_den);
            $query->andFilterWhere(['<=', 'ngay_nhan_ao', $this->ngay_nhan_ao_den]);
        }
