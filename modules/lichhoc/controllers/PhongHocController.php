...
use app\modules\lichhoc\models\search\PhongHocSearch;
...
class PhongHocController extends Controller
{
            ...
            if($request->isGet){
               ...
               if($model->file) {
                   ...
                   if($model->file->saveAs($filePath)) {
                       $model->so_do_phong = 'images/' . $fileName;
                       $model->save(false); 
                   }
                }
                return [
                    ...
                    'title'=> "Thêm phòng học",
                    'content'=>'<span class="text-success">Thêm mới phòng thành công !</span>',
                    'footer'=> Html::button('Đóng lại',['class'=>'btn btn-default pull-left','data-bs-dismiss'=>"modal"]).
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing PhongHoc model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
